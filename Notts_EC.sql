...
DROP TABLE IF EXISTS NHSE_Sandbox_HEU.temp.COPD_snomed 
CREATE TABLE NHSE_Sandbox_HEU.temp.COPD_snomed 
(snomed_ct NVARCHAR(20) NULL)
INSERT INTO NHSE_Sandbox_HEU.temp.COPD_snomed 
VALUES 
...
WHERE  diag.EC_Diagnosis_01 IN (select snomed_ct from NHSE_Sandbox_HEU.temp.COPD_snomed) OR 
diag.EC_Diagnosis_02 IN (select snomed_ct from NHSE_Sandbox_HEU.temp.COPD_snomed) AND
...
DROP TABLE IF EXISTS NHSE_Sandbox_HEU.temp.Notts_EC
SELECT 
...
INTO NHSE_Sandbox_HEU.temp.Notts_EC
FROM [dbo].[tbl_Data_SUS_EC] as ec
left join [dbo].[tbl_Data_SUS_EC_2122_Cost]as cost
on cost.EC_Ident = ec.EC_Ident 
left join [dbo].[tbl_Data_SUS_EC_Diagnosis] as diag
...
ec.GP_Practice_Code IN (SELECT GP_code FROM NHSE_Sandbox_HEU.temp.HEU_097_Notts_GP)
...
select 
	...
	ICP_Name
from NHSE_Sandbox_HEU.temp.Notts_EC
Group by ICP_Name
